##########################################################################################################################
#对一些部分重要题目进行总结
#题目7，15，25-28.窗口函数rank（）rownumber（）denserank（）以及聚合函数，日期函数的运用（最长登陆天数，留存率）
#题目3.对数据截取
#题目4，14.寻找最小最大值
#题目6：条件求和sum（case when）求和
#题目8：连续登录天数（rownumbder和date_sub结合）
#题目9，23：字符串粘贴 concat（）groupconcat（）
#题目10：日期函数综合运用
#题目12，21.查询某个字符在某个数据中的出现次数
#题目13.时间函数使用
#题目18-20：数据库表格创建数据插入以及创建索引
#题目30.累积的计算方法
#题目31，32.向上向下取整
##########################################################################################################################

##########################################################################################################################
#1.现在运营想查看每个学校用户的平均发贴和回帖情况，寻找低活跃度学校进行重点运营，请取出平均发贴数低于5的学校或平均回帖数小于20的学校。
##########################################################################################################################
...
############################################################################################################################
#2.现在运营想要查看用户在某天刷题后第二天还会再来刷题的平均概率。请你取出相应数据。
############################建表##############################################################################################
drop table if exists `user_profile`;
...
############################################################################################################################
#3.现在运营举办了一场比赛，收到了一些参赛申请，表数据记录形式如下所示，现在运营想要统计每个性别的用户分别有多少参赛者，请取出相应结果
############################建表##############################################################################################
...
##############################################################
#4.现在运营想要找到每个学校gpa最低的同学来做调研，请你取出每个学校的最低gpa。
##############################################################
drop table if exists user_profile;
CREATE TABLE `user_profile` (
`id` int NOT NULL,
...
##############################################################
#5.现在运营想要了解复旦大学的每个用户在8月份练习的总题目数和回答正确的题目数情况，请取出相应明细数据，对于在8月份没有练习过的用户，答题数结果返回0.
##############################################################
drop table if exists `user_profile`;
drop table if  exists `question_practice_detail`;
drop table if  exists `question_detail`;
...
##############################################################
#6.现在运营想要了解浙江大学的用户在不同难度题目下答题的正确率情况，请取出相应数据，并按照准确率升序输出。
##############################################################
drop table if exists `user_profile`;
drop table if  exists `question_practice_detail`;
drop table if  exists `question_detail`;
...
##############################################################
#题目7.描述从听歌流水中找到18-25岁用户在2022年每个月播放次数top 3的周杰伦的歌曲
##############################################################
...
##############################################################
#10.现需要查询 Tom 这个客户在 2023 年每月的消费金额（按月份正序显示）
##############################################################
...
##############################################################
#15.请你查找employees里入职员工时间排名倒数第三的员工所有信息
##############################################################
drop table if exists  `employees` ;
CREATE TABLE `employees` (
`emp_no` int(11) NOT NULL,
...
##############################################################
#16.获取每个部门中当前员工薪水最高的相关信息，给出dept_no, emp_no以及其对应的salary，按照部门编号dept_no升序排列
##############################################################
...
##########################################################
#22.请你将employees中的first_name，并按照first_name最后两个字母升序进行输出
##########################################################
...
##########################################################
#23.按照dept_no进行汇总，属于同一个部门的emp_no按照逗号进行连接，结果给出dept_no以及连接出的结果
##########################################################
...
##########################################################
#24.查找排除在职(to_date = '9999-01-01' )员工的最大、最小salary之后，其他的在职员工的平均工资avg_salary
##########################################################
